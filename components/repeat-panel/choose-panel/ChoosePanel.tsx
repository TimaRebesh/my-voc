...
export default function ChoosePanel(props: ChooseProps) {
  ...
  useEffect(() => {
    const callback = (e: any) => {
      if (e.target.localName === 'button') e.target.focus();
    };
  ...
  useEffect(() => {
    if (chosenID) nextButtonRef.current?.focus();
  }, [chosenID]);
  ...
  const buttonStatus = (id: string) => {
    ...
    if (
      chosenID &&
      chosenID !== props.studyWord.id &&
      props.studyWord.id === id
    )
  ...
  const keyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === 'ArrowDown') {
      ...
      if (elFocusID === props.optionalWords.length - 1)
        (
          (buttonsGroupRef.current as HTMLDivElement)
