...
export default function StudyNewPanel({ vocabulary, user }: StudyPanelProps) {
  ...
  useEffect(() => {
    if (vocabulary.list.length === 0) return;
    const listToStudy = prepareListToStudy(vocabulary.list);
    repeatWatcher.current = listToStudy.map((word) => ({
      id: word.id,
      repeated: 0,
    }));
    setStudiedOrder(0);
  ...
  useEffect(() => {
    if (studiedOrder < 1000 && dataSet.length > 0) defineWords();
  }, [studiedOrder]);
  ...
  const prepareListToStudy = (collection: Word[]) => {
    ...
    filteredByNew.sort((a, b) =>
      getWordProgress(a) < getWordProgress(b) ? 1 : -1
    );
    ...
    if (shuffledData.length === 1) shuffledData.push(mockWord);
    return shuffledData;
  };
  ...
  const checkIsWordSuitable = (order: number) => {
    ...
    const isRepeatedTimeNotFinished =
      (repeatWatcher.current.find((w) => w.id === currentWord.id) as WordState)
        .repeated < 3;
  ...
  const update = (isCorrectAnswer: boolean) => {
    setNextWordOrder();
    changeWatcher();
    if (isCorrectAnswer) updateDataSet();
  };

  const setNextWordOrder = () =>
    setStudiedOrder(dataSet.length - 1 === studiedOrder ? 0 : studiedOrder + 1);

  const changeWatcher = () => {
    repeatWatcher.current.forEach((rw) => {
      if (rw.id === studiedWord?.id) rw.repeated++;
    });
  };
  ...
  const updateDataSet = () => {
    if (studiedWord) {
      ...
      const updatedData = dataSet.map((e) =>
        e.id === studiedWord.id ? studiedWord : e
      );
  ...
  const getPanel = () => {
    ...
    if (dataSet.length === 0)
      return (
        <MessagePanel
          legend={'You learned all words'}
          messages={['Please add new words in "My vocabulary"']}
        />
      );
    else if (isStudyFinished)
      return (
        <MessagePanel messages={['Study is finished']}>
